...
/**
 * The raw public API of the presentation compiler that does not handle synchronisation. Scala
 * compiler can't run concurrent code at that point, so we need to enforce sequential, single
 * threaded execution. It has to be implemented by the consumer of this API.
 *
...
public abstract class RawPresentationCompiler {
   ...
   *
   * <p>The returned completion items are incomplete meaning they may not contain available
   * information such as documentation or the detail signature may have `x$1` parameter names for
   * Java methods. It's recommended to call {@link #completionItemResolve(CompletionItem, String)}
   * before displaying the `detail` and `documentation` fields to the user.
   *
   * @implNote supports cancellation.
   */
  public abstract CompletionList complete(
      OffsetParams params, CompletionTriggerKind completionTriggerKind);

  /**
   * Returns a fully resolved completion item with defined fields such as `documentation` and
   * `details` populated.
   *
  ...
  /**
   * Returns the type of the expression at the given position along with the symbol of the
   * referenced symbol.
   */
  public abstract Optional<HoverSignature> hover(OffsetParams params);

  /**
   * Checks if the symbol at given position can be renamed using presentation compiler. Returns
   * Some(range) if symbol is defined inside a method or the file is a worksheet, None otherwise
   */
  public abstract Optional<Range> prepareRename(OffsetParams params);

  /**
   * Renames the symbol at given position and all of its occurrences to `name`. If symbol can't be
   * renamed using presentation compiler (prepareRename returns None), returns empty list.
   */
  ...
  public abstract <T> List<TextEdit> codeAction(
      OffsetParams params, String codeActionId, Optional<T> codeActionPayload);
  ...
  public abstract List<AutoImportsResult> autoImports(
      String name, OffsetParams params, Boolean isExtension);
  ...
  /**
   * Return named arguments for the apply method that encloses the given position. May fail with a
   * DisplayableException.
   */
  public abstract List<TextEdit> convertToNamedArguments(
